...
fn test_flash_swap_not_enough_liquidity() {
    ...
    let flash_swap_class = declare("FlashSwapTest").unwrap();
    let (flash_swap_address, _) = flash_swap_class.deploy(@flash_swap_constructor_calldata).unwrap();
    ...
    let pair_safe_dispatcher = IPairC1Dispatcher { contract_address: pair_address };

    let mut data = ArrayTrait::<felt252>::new();
    data.append(0);
    start_prank(CheatTarget::One(pair_address), user2());
    pair_safe_dispatcher.swap(amount_token_0, 0, flash_swap_address, data);
