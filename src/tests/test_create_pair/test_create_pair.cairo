...
use jediswap::tests::utils::utils::{ deployer_addr, token0, token1, zero_addr };
...
fn test_create_pair_same_pair() {
    ...
    let _pair_address = factory_dispatcher.create_pair(token0(), token1());
    // let _pair_address = factory_dispatcher.create_pair(token1(), token0());
}
