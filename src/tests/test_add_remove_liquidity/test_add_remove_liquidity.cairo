...
use jediswap::tests::utils::utils::{ deployer_addr, token0, token1, burn_addr, user1, TOKEN_MULTIPLIER, TOKEN0_NAME,
...
fn test_add_liquidity_expired_deadline() {
    ...
    let router_safe_dispatcher = IRouterC1Dispatcher { contract_address: router_address };
    ...
    let (_amountA, _amountB, _liquidity) = router_safe_dispatcher.add_liquidity(token0(), token1(), token_amount, token_amount, 
...
fn test_add_remove_liquidity_created_pair(){
    ...
    start_prank(CheatTarget::One(pair_address), user1());
    start_prank(CheatTarget::One(router_address), user1());
    let (amountA, amountB, _liquidity) = router_dispatcher.add_liquidity(
        token0_address, token1_address, amount_token0, amount_token1, 1, 1, user1(), 0);
    stop_prank(CheatTarget::One(pair_address));
    stop_prank(CheatTarget::One(router_address));
    ...
    start_prank(CheatTarget::One(router_address), user1());
    let (_amountA_burn, _amountB_burn) = router_dispatcher.remove_liquidity(
