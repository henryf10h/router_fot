...
mod RouterC1 {
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn _swap_supporting_fee_on_transfer_tokens(ref self: ContractState, index: u32 , path: Array<ContractAddress>, _to: ContractAddress){
            ...
            let (token0, _token1) = _sort_tokens(*path[index], *path[index + 1]);
            let pair = _pair_for(factory, *path[index], *path[index + 1]);
            let (reserve_0, reserve_1) = _get_reserves(factory, *path[index], *path[index + 1]);
            ...
            let amount_output = _get_amount_out(amount_input, reserve_input, reserve_output);
            ...
            let mut to: ContractAddress = _to;
            if (index < (path.len() - 2)) {
                to = _pair_for(factory, *path[index + 1], *path[index + 2]);
            }
            let data = ArrayTrait::<felt252>::new();
            let pairDispatcher = IPairDispatcher{ contract_address: pair };
